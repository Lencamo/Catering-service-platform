...
const foodMenusIds = ref()

// 监听节点选择
const handleTreeNodeCheck = (arg1, arg2) => {
  foodMenusIds.value = [...arg2.checkedKeys, ...arg2.halfCheckedKeys]
  // console.log(foodMenusIds.value)
}
...
const setRoleDialogVisible = (isNew: boolean, food?: any) => {
  ...
  if (!isNew && food) {
    ...
    editRoleId.value = food.id

    // - 菜单数据
    nextTick(() => {
      const { deepestMenuIds, allMenuIds } = mapMenuToIds(food.menuList)

      foodMenusIds.value = allMenuIds // 初始化foodMenusIds值
...
const handlePassVerify = () => {
  ...
  const payload = { ...dialogData, foodMenusIds: foodMenusIds.value }
  // console.log(payload)
